 ...
 * ***********************************************************************************************************************
 * Example 6. 
 * The example below sets the FlexCAN2 in classic mode plus interrupt. Pins 81 and 80 configured as CAN Tx and Rx. As the 
 * previous reception example, it only waits until a single 8 bytes message is received. For the callback CanIf_RxIndication 
 * function this time the messages accepted will be only those who has the IDs of 0x1EF and 0x022. Two filter have been 
 * configured to accept only one ID, each filter corresponds to one Hardware object made of one element in receive mode. 
...
void CanIf_RxIndication(const Can_HwType* Mailbox, const PduInfoType* PduInfoPtr )
{
    ...
    {
        /*do something in case the first byte received is equal to 0x32*/
        for(uint8 i = 0; i < 8 ; i++)
        {
