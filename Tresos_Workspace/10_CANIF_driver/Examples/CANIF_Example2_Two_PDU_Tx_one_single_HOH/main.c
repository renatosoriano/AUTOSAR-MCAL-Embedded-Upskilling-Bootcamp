 ...
 * example is that we are using the CanIf layer configured for two single Ids message ( 0x123 and 0x124 ), one single 
 * buffer is used with one single Hoh object for both messages, we do not need more. But notice we still use the same 
 * callback CDD_UserTxConfirmation function for notification in both messages or PDUs, previously referenced in Tresos. 
