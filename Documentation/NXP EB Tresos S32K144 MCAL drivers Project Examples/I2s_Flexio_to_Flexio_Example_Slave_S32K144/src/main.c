...
int main(void)
{
    ...
    Port_Init(NULL_PTR);
    Mcl_Init(NULL_PTR);
    /* Init FLEXIO_I2S as Slave */
    I2s_Init(NULL_PTR);
    
    /* Part1: Flexio_I2s Slave receives data */
    ...
    do
    {
     /* Wait until the Slave receives all data */
	...
	/*Flexio_I2s Slave prepare to send data received*/
	I2s_SlaveRequest.bufferSize = BUFF_SIZE;
    I2s_SlaveRequest.dataDirection = I2S_SEND_DATA;
    I2s_SlaveRequest.pDataBuffer = (I2s_DataType (**))RecvData;
	
	/*Flexio_I2s Slave sends data received*/
	pRequest = &I2s_SlaveRequest;
    Ret = I2s_AsyncTransmit(I2S_FLEXIO_SLAVE, pRequest);
	if (E_NOT_OK == Ret)
    {
